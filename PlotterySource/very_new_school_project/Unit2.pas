...
begin
    PointsTool.Caption := 'Points (' + getNum(currentScene.getCurrentLayer().points.size()) + ')';
...
begin
    LinesTool.Caption := 'Lines (' + getNum(currentScene.getCurrentLayer().lines.size()) + ')';
...
procedure TForm2.AddLayer(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
    if currentScene.addLayer(Layer.Create('New Layer')) then begin
        updateTools();
...
begin
    if isInt(RandomizeToolAmount.Text, a) then begin
        ...
        proceed := true;
        if a > rest then
            if customMessagedlg('Only ' + IntToStr(rest) + ' points can be added',
                'Crossroads where our roads diverge', mtCustom,
                [mbYes, mbNo, mbOK], ['Proceed', 'Abort', 'I am against aborts!']) = mrNo then begin
                proceed := false;
            end;
...
begin
    ...
    if isInt(CutToolFrom.Text, min) and
       isInt(CutToolTo.Text, max) then begin
        ...
        if ask then
            if customMessagedlg('Only [' + IntToStr(min) + ',' + IntToStr(max) + '] is available',
                'Choice', mtCustom,
                [mbYes, mbNo], ['Proceed', 'Abort']) = mrNo then begin
                proceed := false;
            end;
...
begin
    ...
    if settings.exitAction = 'ask' then begin
        ...
        selected := customMessagedlg('Back to menu or exit app?', 'Exit', mtCustom,
            [mbYes, mbNo, mbOK, mbAll], ['Now: Menu', 'Menu', 'Exit', 'Now: Exit']);
